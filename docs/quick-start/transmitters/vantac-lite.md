...
## Flashing Vantac Lite

Following are the flashing methods for Vantac Lite
...
??? Note "Flashing via WiFi"
    ...
    ??? Note "Method 1"
        ...
        2. Build the firmware, which will open the Target folder where the `Vantac_Lite_2400_TX-<version>.bin` file is located. Do not close this window so you can easily locate the correct file to upload to the module.
        ...
        9. Drag and drop the firmware file, `Vantac_Lite_2400_TX-<version>.bin`, created by the ExpressLRS Configurator. Alternatively, use the `Choose File` button to select the file from the folder where the firmware was created.
    ...
    ??? Note "Method 2"
        ...
        7. Drag-and-drop the `Vantac_Lite_2400_TX-<version>.bin` file created by the ExpressLRS Configurator into the "Choose File" field, or manually navigate to the folder and select the file.
