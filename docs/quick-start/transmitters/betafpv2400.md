...
## Flashing BetaFPV 2.4GHz

Following are the flashing methods for BetaFPV 2.4GHz.
...
??? Note "Flashing via WiFi"
    ...
    ??? Note "Method 1"
        ...
        2. Build the firmware, which will open the Target folder where the `BETAFPV_2400_TX-<version>.bin` or `BETAFPV_2400_TX_MICRO-<version>.bin` file is located. Do not close this window so you can easily locate the correct file to upload to the module.
        ...
        9. Drag and drop the firmware file, `BETAFPV_2400_TX-<version>.bin` or `BETAFPV_2400_TX_MICRO-<version>.bin`, created by the ExpressLRS Configurator. Alternatively, use the `Choose File` button to select the file from the folder where the firmware was created.
    ...
    ??? Note "Method 2"
        ...
        7. Drag-and-drop the `BETAFPV_2400_TX-<version>.bin` or `BETAFPV_2400_TX_MICRO-<version>.bin` file created by the ExpressLRS Configurator into the "Choose File" field, or manually navigate to the folder and select the file.
