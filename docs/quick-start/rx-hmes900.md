...
With the receiver [wired properly](../quick-start/rx-fcprep.md) to your FC, select the right target and set your Firmware Options, then click on **Build and Flash** on the ExpressLRS Configurator. First time Compile naturally takes a while but if you do the prep work properly, you'll be greeted with the `Success` message soon enough!
