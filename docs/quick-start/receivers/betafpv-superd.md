...
## Flashing BetaFPV SuperD 2.4GHz

Following are the flashing methods for BetaFPV SuperD 2.4GHz.
...
??? Note "Flashing via WiFi"
    ...
    ??? Note "Method 1"
        ...
        3. Build the firmware, which will open the Target folder where the `BETAFPV_SuperD_2400_RX-<version>.bin` file is located. Do not close this window so you can easily locate the correct file to upload to the module.
        ...
        7. Drag and drop the firmware file, `BETAFPV_SuperD_2400_RX-<version>.bin`, created by the ExpressLRS Configurator. Alternatively, use the `Choose File` button to select the file from the folder where the firmware was created.
    ...
    ??? Note "Method 2"
        ...
        8. Drag and drop the firmware file, `BETAFPV_SuperD_2400_RX-<version>.bin`, created by the ExpressLRS Configurator. Alternatively, use the `Choose File` button to select the file from the folder where the firmware was created.
...
??? Note "Flashing via FTDI"
    ...
    4. After the `Success` message shows, the RGB LED on the receiver will light up and show the RGB wave pattern. Then it will blink slow in Orange. 5. Receiver is now flashed and waiting for the TX Sync.
