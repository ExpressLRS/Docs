...
## Flashing HGLRC Hermes 900MHz

Following are the flashing methods for HGLRC Hermes 900MHz.
...
??? Note "Flashing via WiFi"
    ...
    ??? Note "Method 1"
        ...
        3. Build the firmware, which will open the Target folder where the `HGLRC_Hermes_900_RX-<version>.bin` file is located. Do not close this window so you can easily locate the correct file to upload to the module.
        ...
        7. Drag and drop the firmware file, `HGLRC_Hermes_900_RX-<version>.bin`, created by the ExpressLRS Configurator. Alternatively, use the `Choose File` button to select the file from the folder where the firmware was created.
    ...
    ??? Note "Method 2"
        ...
        8. Drag and drop the firmware file, `HGLRC_Hermes_900_RX-<version>.bin`, created by the ExpressLRS Configurator. Alternatively, use the `Choose File` button to select the file from the folder where the firmware was created.
