...
## Flashing iFlight 900MHz

Following are the flashing methods for iFlight 900MHz.

### <span class="custom-heading" data-id="1">Flashing via WiFi</span>

??? Note "Flashing via WiFi"

    - Target: `iFlight_900_RX_via_WIFI`

    - Device Category: `iFlight 900 MHz`

    - Device: `iFlight 900 RX`
    ...
    ??? Note "Method 1"
        ...
        3. Build the firmware, which will open the Target folder where the `iFlight_900_RX-<version>.bin` file is located. Do not close this window so you can easily locate the correct file to upload to the module.
        ...
        7. Drag and drop the firmware file, `iFlight_900_RX-<version>.bin`, created by the ExpressLRS Configurator. Alternatively, use the `Choose File` button to select the file from the folder where the firmware was created.
    ...
    ??? Note "Method 2"
        ...
        8. Drag and drop the firmware file, `iFlight_900_RX-<version>.bin`, created by the ExpressLRS Configurator. Alternatively, use the `Choose File` button to select the file from the folder where the firmware was created.
...
??? Note "Flashing via Passthrough"
    ...
    - Target: `iFlight_900_RX_via_BetaflightPassthrough`
...
??? Note "Flashing via FTDI"
    ...
    - Target: `iFlight_900_RX_via_UART`
